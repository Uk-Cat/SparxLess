...
async function postToSupabase(data) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("[Sparx Ext] Supabase POST failed:", message, errorBody);
      return { success: false, error: message };
    }

    const inserted = await response.json();
    console.log("[Sparx Ext] Row inserted:", inserted);
    return { success: true, data: inserted };

  } catch (err) {
    console.error("[Sparx Ext] Network error:", err);
...
chrome.runtime.onInstalled.addListener(() => {
  console.log("[Sparx Ext] Service worker ready.");
});